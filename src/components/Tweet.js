...
export default function Tweet(props) {
    ...
    async function deleteTweetUser(tweetId) {
        ...
        try {
            const respDeletedTweet = await axios.delete(`https://twitter-backend-7a5d.onrender.com/api/vt1/d/${tweetId}`, {
    ...
    async function likeTweets(tweetId) {
        const token = localStorage.getItem('myToken');
        try {
            await axios.get(`https://twitter-backend-7a5d.onrender.com/api/vt1/like/${tweetId}`, {
                headers: { "Authorization": "Bearer " + token }
            });
            getAllFeeds();
            getLikedTweet();
    ...
    async function getLikedTweet() {
        const token = localStorage.getItem('myToken');
        try {
            const respLikedTweets = await axios.get('https://twitter-backend-7a5d.onrender.com/api/vt1/details', {
                headers: { "Authorization": "Bearer " + token }
            })
    ...
    async function addRetweet(tweetId) {
        const token = localStorage.getItem("myToken");
        try {
            await axios.get(`https://twitter-backend-7a5d.onrender.com/api/vt1/rt/${tweetId}`, {
                headers: { "Authorization": "Bearer " + token }
            });
            getAllFeeds();
            getLikedTweet();
